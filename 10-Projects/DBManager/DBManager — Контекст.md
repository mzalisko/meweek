...
**Оновлено:** 2026-06-15
**Стан:** Плани 1.1 (Core), 1.2 (DataBridge), 1.3 (WP-плагін з гео) ВИКОНАНО. Етап 2: **плани 2.1–2.4 виконано**. 2.1 — вхід (Breeze, Livewire+Tailwind), оболонка `/admin`, грід «Значення». 2.2 — панель слота (pin/unpin, режим повернення, вичерпання, месенджери, «Зберегти»→публікація). 2.3 — редактор значень: CRUD, **область дії група (багато сайтів) / сайт (один/перекриття)**, перекриття групового, публікація уражених. 2.4 — телефонний слот: створення phone-значення (DataValue+PhoneSlot) + керування номерами в панелі (додати/видалити/↑↓ пріоритет, recompute, аудит). Core **143 тести**. Далі — план 2.5 (bulk над виділеним з прев'ю «N сайтів»/відкатом; перевикористання спільних номерів; drag-drop), потім дашборд/інциденти/ролі/імпорт. Огляд для людини — `documentation/`.
