...
**Оновлено:** 2026-06-15
**Стан:** Плани 1.1 (Core), 1.2 (DataBridge), 1.3 (WP-плагін з гео) ВИКОНАНО. Етап 2: **плани 2.1 і 2.2 виконано**. 2.1 — вхід (Breeze, Livewire+Tailwind), оболонка `/admin`, грід «Значення» (failover-статуси, область дії, фільтри, перемикач сайта). 2.2 — панель слота: pin/unpin, режим повернення (Авто/Sticky), політика вичерпання, перемикання месенджерів, «Зберегти»→публікація уражених сайтів у bridge. Core **106 тестів**. Далі — план 2.3 (дашборд, інциденти, аудит-стрічка з відкатом, ролі/права, імпорт CSV/XLSX; drag-реордер резервів). Огляд для людини — `documentation/`.
