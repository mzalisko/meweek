...
**Оновлено:** 2026-06-15
**Стан:** Плани 1.1 (Core), 1.2 (DataBridge), 1.3 (WP-плагін з гео) ВИКОНАНО. Етап 2: **плани 2.1–2.6 виконано**. 2.1–2.4 — адмінка, грід, панель слота, редактор, телефонний слот. 2.5 — виділення рядків чекбоксами ✅ (bulk прев'ю+видалення — **на паузі**, Tasks 2–4). 2.6 — **ValueEditor → фіксована права панель** (замість модалки), взаємне виключення зі SlotPanel через Livewire-події, гео-мітки (toggle-чіпи + sync + аудит), linked_slot для месенджера. Core **164 тести**. Далі — добити план 2.5 (прев'ю «N сайтів» + bulk-видалення), потім дашборд/інциденти/ролі/імпорт. Огляд для людини — `documentation/`.
