...
**Оновлено:** 2026-06-15
**Стан:** Плани 1.1 (Core), 1.2 (DataBridge), 1.3 (WP-плагін з гео) ВИКОНАНО. Етап 2: **план 2.1 (адмінка) виконано** — вхід (Breeze, Livewire+Tailwind) + оболонка `/admin` + грід «Значення» (failover-статуси, область дії група/сайт, фільтри тип/гео/статус/пошук, перемикач сайта). Core 84 тести. Далі — план 2.2 (панель слота: редагування ланцюга номерів, pin, повернення, вичерпання, месенджери, Save→аудит+публікація). Огляд для людини — `documentation/`.
