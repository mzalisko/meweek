...
**Оновлено:** 2026-06-15
**Стан:** Плани 1.1 (Core), 1.2 (DataBridge), 1.3 (WP-плагін з гео) ВИКОНАНО. Етап 2: **плани 2.1, 2.2, 2.3 виконано**. 2.1 — вхід (Breeze, Livewire+Tailwind), оболонка `/admin`, грід «Значення» (failover-статуси, область дії, фільтри, перемикач сайта). 2.2 — панель слота (pin/unpin, режим повернення, вичерпання, месенджери, «Зберегти»→публікація). 2.3 — редактор значень: додати/редагувати/видалити, **область дії група (багато сайтів) / сайт (один/перекриття)**, перекриття групового для сайта, публікація уражених. Core **130 тестів**. Далі — план 2.4 (створення телефонного слота + номери; bulk над виділеним з прев'ю/відкатом; drag-реордер резервів), потім дашборд/інциденти/ролі/імпорт. Огляд для людини — `documentation/`.
