...
add_action('init', function (): void {
    ...
    if (! function_exists('dbm_get_price')) {
        function dbm_get_price(string $key, array $opts = []): string
        {
            ...
            $class = ! empty($priceCls) ? ' class="' . htmlspecialchars($priceCls, ENT_QUOTES) . '"' : '';
    ...
    if (! function_exists('dbm_get_phone_block')) {
        function dbm_get_phone_block(string $key, array $opts = []): string
        {
            ...
            if ($classPhone !== '') {
                $extraCls .= ' ' . htmlspecialchars($classPhone, ENT_QUOTES);
            }
            ...
            if (count($messengers) > 0) {
                ...
                foreach ($messengers as $msg) {
                    ...
                    if ($msgUrl !== '') {
                        ...
                        if ($customImg !== '') {
                            $icon = '<img class="dbm-phone-block__msg-icon dbm-phone-block__msg-icon--' . htmlspecialchars($net, ENT_QUOTES) . '" src="' . htmlspecialchars($customImg, ENT_QUOTES) . '" alt="' . htmlspecialchars($net, ENT_QUOTES) . '" style="width: 18px; height: 18px; object-fit: contain; vertical-align: middle;" />';
                        } elseif ($net === 'telegram') {
                        ...
                        $msgLinkCls = 'dbm-phone-block__msg-link dbm-phone-block__msg-link--' . htmlspecialchars($net, ENT_QUOTES);
                        if ($classMessenger !== '') {
                            $msgLinkCls .= ' ' . htmlspecialchars($classMessenger, ENT_QUOTES);
                        }
                        $html .= '<a class="' . $msgLinkCls . '" href="' . htmlspecialchars($msgUrl, ENT_QUOTES) . '" target="_blank" rel="noopener noreferrer" title="' . $msgName . '">' . $icon . '</a>';
                    }
                }
                $html .= '</div>';
            }
